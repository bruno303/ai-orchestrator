...
def _seed(remote_repo: str, issue_number: int = 1) -> dict:
    ...
    return {
        "task_id": task_id,
        "input": {"provider": "github", "data": {
            "id": task_id, "repository": "company/backend", "title": "Add a feature",
            "description": "Please implement the feature.", "input_provider": "github",
...
def test_disallowed_repository(remote_repo, store):
    ...
    seed["input"]["data"]["repository"] = "evil/repo"
...
def test_executor_reported_failure_fails_even_with_zero_exit_code(remote_repo, allowlist, store):
    class FailingExecutor:
        def execute(self, request):
            return ExecutionResult(False, 0, context=Context({"opencode": {"executor_run": "rejected"}}))
    ...
    class FakeExecutor:
        def execute(self, request):
            ...
            if request.agent == "plan":
                return ExecutionResult(True, 0, stdout="# Plan\n\nDo it.", context=request.context.merge_namespace("opencode", {"executor_run": "plan"}))
            if request.agent == "build":
                if "Implement work item" in request.prompt:
                    Path(request.workspace, "work.txt").write_text("implemented\n")
                    return ExecutionResult(True, 0, stdout="implemented", context=request.context.merge_namespace("opencode", {"executor_run": "implement"}))
                return ExecutionResult(True, 0, stdout="tested", context=request.context.merge_namespace("opencode", {"executor_run": "test"}))
    ...
    class FakeWorkspaceManager:
        def prepare(self, request):
            ...
            return WorkspaceResult(
                ...
                Context({"custom_workspace": {"base_branch": "main", "provider_token": "keep-me"}}),
            )

        def cleanup(self, result):
            calls.append("cleanup")
            assert result.context.namespace("custom_workspace")["provider_token"] == "keep-me"

    class FakeDestination:
        def publish(self, request):
            calls.append("publish")
            return PublishedChange("99")
...
def test_prepare_workspace_uses_manager_default_when_base_branch_is_missing(allowlist, tmp_path):
    class FakeWorkspaceManager:
        def prepare(self, request):
            ...
            return WorkspaceResult(
                ...
                Context({"custom_workspace": {"base_branch": "develop", "provider_token": "resolved"}}),
                "develop",
            )
    ...
    seed["input"]["data"]["context"]["git"].pop("base_branch")
    result = prepare_workspace(seed, FakeWorkspaceManager())

    assert result["workspace"]["base_branch"] == "develop"
    assert result["workspace"]["context"]["custom_workspace"] == {"base_branch": "develop", "provider_token": "resolved"}


def test_prepare_workspace_preserves_requested_base_branch(allowlist, tmp_path):
    class FakeWorkspaceManager:
        def prepare(self, request):
            return WorkspaceResult(str(tmp_path / "workspace"), request.branch, Context(), "main")
...
def test_injected_workspace_cleanup_failure_does_not_fail_task(tmp_path, allowlist, store):
    ...
    class WorkspaceManager:
        def prepare(self, request):
            ...
            return WorkspaceResult(
                str(workspace_path), request.branch,
                Context({"custom_workspace": {"base_branch": "main", "provider_token": "lifecycle"}}), "main",
            )

        def cleanup(self, result):
            assert result.context.namespace("custom_workspace")["provider_token"] == "lifecycle"
...
def test_create_pr_with_existing_commit(remote_repo, allowlist, store, monkeypatch):
    ...
    git_mod.create_worktree(repo_dir, ws, "ai/issue-4", "main")
    (ws / "work.txt").write_text("hello\ncommitted\n")
    git_mod.commit_all(ws, "feat: agent committed\n\nCloses #4")
    seed.update({"workspace": {"path": str(ws), "branch": "ai/issue-4", "base_branch": "main"}, "status": state_mod.CREATING_PR})
...
def test_create_pr_reuses_existing_pr(remote_repo, allowlist, store, monkeypatch):
    ...
    repo_dir = git_mod.ensure_base_clone("company/backend", f"file://{remote_repo}")
    git_mod.create_worktree(repo_dir, ws, "ai/issue-5", "main")
    (ws / "work.txt").write_text("hello\ncommitted\n")
    git_mod.commit_all(ws, "feat: agent committed\n\nCloses #5")
    seed.update({"workspace": {"path": str(ws), "branch": "ai/issue-5", "base_branch": "main"}, "status": state_mod.CREATING_PR})
...
def test_create_pr_reuse_normalized_does_not_edit(remote_repo, allowlist, store, monkeypatch):
    ...
    repo_dir = git_mod.ensure_base_clone("company/backend", f"file://{remote_repo}")
    git_mod.create_worktree(repo_dir, ws, "ai/issue-7", "main")
    (ws / "work.txt").write_text("hello\ncommitted\n")
    git_mod.commit_all(ws, "feat: agent committed\n\nCloses #7")
    seed.update({"workspace": {"path": str(ws), "branch": "ai/issue-7", "base_branch": "main"}, "status": state_mod.CREATING_PR})
