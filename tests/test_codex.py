...
def test_codex_discussion_failure_does_not_publish_stdout(tmp_path, monkeypatch):
    monkeypatch.setenv("FAKE_CODEX_FAIL", "1")

    result = codex.CodexDiscussionExecutor().execute(
...
def test_codex_review_failure_preserves_both_streams(tmp_path, monkeypatch):
    monkeypatch.setenv("FAKE_CODEX_FAIL", "1")
...
def test_codex_triage_failure_preserves_both_streams(tmp_path, monkeypatch):
    monkeypatch.setenv("FAKE_CODEX_FAIL", "1")

    result = codex.CodexTriageExecutor().execute(
